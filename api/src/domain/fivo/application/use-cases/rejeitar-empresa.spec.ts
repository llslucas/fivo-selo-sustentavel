...
describe('RejeitarEmpresaUseCase', () => {
  let rejeitarEmpresaUseCase: RejeitarEmpresaUseCase;
  let empresaRepository: EmpresaRepository;

  beforeEach(() => {
    empresaRepository = new InMemoryEmpresaRepository();
    rejeitarEmpresaUseCase = new RejeitarEmpresaUseCase(empresaRepository);
  });

  it('should reject an existing empresa if the user is an admin', async () => {
    ...
    await rejeitarEmpresaUseCase.execute(mockEmpresa.id.toString(), mockUser);

    const updatedEmpresa = await empresaRepository.findById(
      mockEmpresa.id.toString(),
    );

    expect(updatedEmpresa?.status).toBe(EmpresaStatus.REJEITADA);
  ...
  it('should throw a NotAllowedError if the user is not an admin', async () => {
    ...
    await expect(
      rejeitarEmpresaUseCase.execute(mockEmpresa.id.toString(), mockUser),
  ...
  it('should throw a ResourceNotFound Error if the empresa not exists.', async () => {
    ...
    await expect(
      rejeitarEmpresaUseCase.execute('non-existent-id', mockUser),
