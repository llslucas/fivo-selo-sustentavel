...
| `CredenciaisInvalidasError.status` é `static readonly` enquanto os demais erros usam `readonly` de instância | `.../errors/wrong-credentials.error.ts:2` | `DomainExceptionFilter` não acha `erro.status` de instância → cai no 500 | Uniformizar: `status` sempre de instância; um teste do filtro cobre cada erro |
